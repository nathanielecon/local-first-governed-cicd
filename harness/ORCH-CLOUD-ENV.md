...
4. Cloud Agents **edit the repo**; they are **not** the AWS/GitOps apply control plane. Live `terraform apply` / destroy and AWS mutation stay on CI / human-gated GitOps only — never treat an agent session as apply authority (see `docs/architecture/phase-9-aws.md`).
